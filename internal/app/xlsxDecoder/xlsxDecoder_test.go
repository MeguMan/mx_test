...
func TestParseFile(t *testing.T) {
	...
	rs := model.RowsStats{}
	oo, err := ParseFile(&rs, "testing", 999)
	assert.NoError(t, err)
	assert.NotNil(t, oo)
	assert.Equal(t, tRs, rs)
}
...
func TestDownloadFile(t *testing.T) {
	...
	err := DownloadFile(url, "downloadTesting")
	assert.NoError(t, err)
	assert.FileExists(t, "downloadTesting.xlsx")
	os.Remove("downloadTesting.xlsx")
}